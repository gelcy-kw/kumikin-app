...
def is_off_or_vacation(task_code):
    c = clean_str(task_code)
    return c in ['公休', '有休', '無休', '特休', '休', 'OFF', 'VACATION', '']

def normalize_area_dynamic(area_val):
    s = clean_str(area_val)
    if not s or s in ['ANY', 'ALL', '共通', 'フリー', '全域', 'NONE', '一括']:
        return 'ANY'
    return s

def parse_trade_allowed(val):
    s = clean_str(val)
    if s in ['N', 'NO', '0', 'FALSE', '不可', '不可（固定）']:
        return 'N'
    return 'Y'
...
def run_optimization(df_members, df_tasks, df_initial_raw):
    ...
    try:
        ...
        # UNNAMED列（余計な空列）や空文字を除外して正しい日付だけを抽出
        dates = []
        for c in all_cols:
            c_clean = clean_str(c)
        ...
        if 'TaskID' in df_tasks.columns:
            for _, row in df_tasks.iterrows():
                ...
                # Load値の取得（未設定や取得失敗時はデフォルトで 2 と判定）
                t_load = parse_int_safely(row.get('Load', 2))
        ...
        LOAD_GAP_PENALTY_WEIGHT = 20  # 遅早と同等のペナルティ
        ...
        # 2. Loadギャップ（1↔3のトレード）ペナルティ
        for p in existing_members:
            for d in dates:
                if day_lock_flags.get(d, False):
                    continue
                orig_t = initial_assignment.get((p, d), '公休')
                orig_load = task_load_map.get(orig_t, 2)
                
                for t in all_tasks:
                    if is_trade_allowed(t) and t != orig_t:
                        new_load = task_load_map.get(t, 2)
                        # Load差が 2（つまり1↔3のトレード）の場合にペナルティ加算
                        if abs(orig_load - new_load) == 2:
                            objective_terms.append(x[p, d, t] * LOAD_GAP_PENALTY_WEIGHT)
...
st.set_page_config(page_title="組換最適化アプリ", layout="wide")

st.title("🧩 組換最適化アプリケーション")
st.markdown("メンバー・タスク・初期勤務表データをアップロードして、最適化を実行します。")

st.sidebar.header("📁 ファイルアップロード")
file_members = st.sidebar.file_uploader("メンバーマスター (CSV)", type=["csv"])
file_tasks = st.sidebar.file_uploader("タスクマスター (CSV)", type=["csv"])
file_initial = st.sidebar.file_uploader("初期勤務表 (CSV)", type=["csv"])

if file_members and file_tasks and file_initial:
    try:
        df_members = pd.read_csv(file_members)
        df_tasks = pd.read_csv(file_tasks)
        df_initial_raw = pd.read_csv(file_initial)

        st.subheader("📊 入力データ確認")
        tab1, tab2, tab3 = st.tabs(["メンバーマスター", "タスクマスター", "初期勤務表"])
        with tab1:
            st.dataframe(df_members.head())
        with tab2:
            st.dataframe(df_tasks.head())
        with tab3:
            st.dataframe(df_initial_raw.head())

        if st.button("🚀 最適化を実行する", type="primary"):
            with st.spinner("最適化計算中..."):
                df_result, success, msg, change_logs, pair_logs, triple_logs, changed_cells, overflow_cells, id_col_name, day_lock_flags, debug_logs = run_optimization(df_members, df_tasks, df_initial_raw)

            if success:
                st.success("🎉 最適化が正常に完了しました！")

                st.subheader("📋 最適化結果")
                
                # スタイリング表示関数
                def highlight_cells(val, row_name, col_name):
                    if (row_name, col_name) in changed_cells:
                        return 'background-color: #ffcccc; color: black; font-weight: bold;'
                    if (row_name, col_name) in overflow_cells:
                ...
                # 結果ログ表示
                st.subheader("📜 変更・トレード詳細ログ")
                
                if triple_logs:
                    with st.expander("🔄 3連番一括トレード成立ログ", expanded=True):
                        for l in triple_logs:
                            st.write(l)

                with st.expander("🔄 トレード変更箇所一覧", expanded=True):
                    if change_logs:
    ...
    st.info("サイドバーから3つのCSVファイルをアップロードしてください。")